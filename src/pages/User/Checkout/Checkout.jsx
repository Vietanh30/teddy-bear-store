        ...
        <>
            ...
            <div className="container mx-auto mt-4">
                <nav className="text-sm flex items-center gap-2">
                    <Link to={path.home} className="hover:text-[#02c4c1]">Trang chủ</Link>
                    <span>&gt;</span>
                    <Link to={path.cart} className="hover:text-[#02c4c1]">Giỏ hàng</Link>
                    <span>&gt;</span>
                    <span>Đặt hàng</span>
                </nav>

                <div className="text-2xl font-bold text-[#ff6683] uppercase mt-4">Đặt hàng</div>
            ...
            <div className="container mx-auto mt-4 text-[#ff6683]">
                {/* Return to cart button */}
                <button
                    onClick={handleReturnToCart}
                    className="mb-4 bg-[#02c4c1] text-white px-4 py-2 rounded-md hover:bg-[#029e9c] flex items-center"
                >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="col-span-1 border p-4 rounded">
                        ...
                        <input type="text" name="fullName" value={formData.fullName}
                            onChange={handleChange} className="w-full border px-3 py-2"
                            placeholder="Nhập họ và tên" />

                        <input type="text" name="phoneNumber" value={formData.phoneNumber}
                            onChange={handleChange} className="w-full border px-3 py-2 mt-3"
                            placeholder="Nhập số điện thoại" />

                        {/* Chọn Tỉnh/Thành phố */}
                        <select name="city" value={formData.city} onChange={handleChange}
                            className="w-full border px-3 py-2 mt-3">
                        ...
                        <select name="district" value={formData.district} onChange={handleChange}
                            className="w-full border px-3 py-2 mt-3" disabled={!formData.city}>
                        ...
                        <select name="ward" value={formData.ward} onChange={handleChange}
                            className="w-full border px-3 py-2 mt-3" disabled={!formData.district}>
                        ...
                        <textarea name="notes" value={formData.notes} onChange={handleChange}
                            className="w-full border px-3 py-2 mt-3 h-20" placeholder="Nhập ghi chú (nếu có)"></textarea>
                    </div>
                                    ...
                                    <div key={item.id} className="flex gap-2 py-2 border-b">
                                        ...
                                        <div className="flex-grow">
                                            ...
                                            <div className="flex justify-between">
                                                <span className="text-[#ff0000] font-bold">{item.price.toLocaleString()} đ x {item.quantity}</span>
                                                <span className="text-[#ff0000] font-bold text-xl">{(item.price * item.quantity).toLocaleString()} đ</span>
                                            </div>
                                        </div>
                                    </div>
                                ))}
