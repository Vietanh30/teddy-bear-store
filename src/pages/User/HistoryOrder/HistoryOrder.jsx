...
function HistoryOrder() {
    ...
    // Render order item
    const renderOrderItem = (item, APP_URL) => {
        ...
        return (
            <div key={item.id} className="flex items-center gap-4 py-3 border-b last:border-b-0">
                <img
                    src={`${APP_URL}${imagePath}`}
                    alt={product.name}
                    className="w-16 h-16 object-cover"
                />
                <div className="flex-grow">
                    <div className="text-[#ff6683] font-medium">{product.name}</div>
    ...
    return (
        <>
            ...
            <div className="container mx-auto mt-4 pb-8">
                ...
                {/* Title */}
                <div className="text-2xl font-bold text-[#ff6683] uppercase mt-4">Lịch sử đơn hàng</div>
                ...
                {orders.length > 0 ? (
                    <div className="mt-4">
                        {orders.map((order) => (
                            <div key={order.id} className="mb-8 border rounded-lg shadow-sm overflow-hidden">
                                ...
                                <div className="bg-gray-50 p-4 border-b flex flex-col md:flex-row justify-between">
                                    <div>
                                        <div className="font-medium text-gray-700">
                                            Mã đơn hàng: <span className="text-[#ff6683]">{order.order_number}</span>
                                        </div>
                                ...
                                {/* Product list */}
                                <div className="p-4">
                                    {order.items.map((item) => renderOrderItem(item, APP_URL))}
                                </div>
                                ...
                                <div className="bg-gray-50 p-4 border-t flex flex-col md:flex-row justify-between items-start md:items-center">
                                    ...
                                    <div className="mt-3 md:mt-0 flex flex-col items-end">
                                        <div className="text-lg font-bold text-[#ff0000]">
                                            Tổng tiền: {parseFloat(order.total_amount).toLocaleString()} đ
                                        </div>
                                        <div className="flex gap-2 mt-2">
                                            {canCancelOrder(order) && (
                                                <button
                                                    ...
                                                    className={`bg-red-500 text-white px-3 py-2 rounded-md text-sm flex items-center gap-1 hover:bg-red-600 transition-colors ${cancellingOrderId === order.id ? 'opacity-70 cursor-not-allowed' : ''}`}
                                                >
                                            ...
                                            {canPayOrder(order) && (
                                                <button
                                                    ...
                                                    className={`bg-[#ff6683] text-white px-3 py-2 rounded-md text-sm flex items-center gap-1 hover:bg-[#02c4c1] transition-colors ${processingOrderId === order.id ? 'opacity-70 cursor-not-allowed' : ''}`}
                                                >
                    ...
                    <div className="py-10 flex flex-col items-center">
                        <div className="text-center text-lg text-[#ff6683] mb-4">Bạn chưa có đơn hàng nào!</div>
